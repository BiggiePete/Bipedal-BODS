...
# Settings of gait
r = 1 # radius of gait shape (can become a function later if we choose)
...
class WalkCycle:
    ...
    def stretchDis(theta):
        xsq = math.pow(WalkCycle.fx(theta), 2)
        ysq = math.pow(WalkCycle.fy(theta), 2)
        return math.sqrt(xsq + ysq)

    # returns target angle of knee in radians
    def kneeAngle(theta):
        dis = WalkCycle.stretchDis(theta)
    ...
    while(True):
        ...
        hipangle2 = map(math.degrees(hipAngle(thetaRad + math.pi)),0,270,0,180) 
        kneeangle2 = map(math.degrees(kneeAngle(thetaRad+ math.pi)),0,270,0,180) 
